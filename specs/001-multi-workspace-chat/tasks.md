...
-   [ ] T002 Install core dependencies: `@supabase/supabase-js`, `@supabase/ssr`, `@tanstack/react-query`, `zustand`, `zod`
...
-   [ ] T004 [P] Configure ESLint + Prettier in `.eslintrc.js` and `.prettierrc`
    -   **Done when**: `pnpm lint` runs without config errors
-   [ ] T005 Create `.env.local.example` with `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_ANON_KEY`
...
-   [ ] T006 Create `supabase/migrations/001_workspaces.sql` with workspaces + workspace_members tables
    -   **Done when**: Migration file contains CREATE TABLE for both, with PK/FK constraints
-   [ ] T007 Create `supabase/migrations/002_channels.sql` with channels table and (workspace_id, name) unique constraint
    -   **Done when**: Migration file contains CREATE TABLE with UNIQUE constraint
-   [ ] T008 Create `supabase/migrations/003_messages.sql` with messages table per data-model.md
...
-   [ ] T009 Add indexes to messages table in `supabase/migrations/003_messages.sql`
    -   `idx_messages_channel_created` on `(channel_id, created_at DESC)`
    -   `idx_messages_user` on `(user_id)`
    -   **Done when**: Both indexes created
-   [ ] T010 Add `updated_at` trigger in `supabase/migrations/003_messages.sql`
    -   **Done when**: Trigger auto-updates `updated_at` on UPDATE

### RLS Policies

-   [ ] T011 Create `supabase/migrations/004_rls_workspaces.sql` with RLS for workspaces + workspace_members
...
-   [ ] T012 Create `supabase/migrations/005_rls_channels.sql` with RLS for channels
    -   SELECT: workspace members can read channels
    -   INSERT: workspace admins can create channels
    -   **Done when**: RLS enabled, policies created
-   [ ] T013 Create `supabase/migrations/006_rls_messages.sql` with RLS for messages per plan.md Section 3
...
-   [ ] T014 Enable realtime publication in `supabase/migrations/006_rls_messages.sql`
...
-   [ ] T016 Create `src/lib/supabase/client.ts` with browser Supabase client (anon key only)
    -   **Done when**: `createBrowserClient()` exported, uses `NEXT_PUBLIC_*` env vars
-   [ ] T017 Create `src/lib/supabase/server.ts` with server Supabase client (cookies)
    -   **Done when**: `createServerClient()` exported for Server Components
-   [ ] T018 Create `src/lib/supabase/middleware.ts` with auth session refresh middleware
    -   **Done when**: Middleware refreshes session on each request
-   [ ] T019 Create `src/middleware.ts` that uses the auth middleware for protected routes
    -   **Done when**: Unauthenticated users redirected to login

### Type Generation

-   [ ] T020 Run `supabase gen types typescript` and save to `src/types/database.ts`
    -   **Done when**: File contains generated types for all tables

### Validation Schemas

-   [ ] T021 Create `src/lib/schemas/message.ts` with Zod schemas per contracts/messages.md
...
-   [ ] T022 [US4] Create `src/services/messages.ts` with `fetchMessages(channelId, cursor?, limit?)` function
    -   Uses cursor pagination (created_at < cursor)
    -   Orders by created_at DESC
    -   Default limit 50
    -   **Done when**: Function returns array of messages, respects cursor
-   [ ] T023 [US4] Create `src/hooks/useMessages.ts` with TanStack `useInfiniteQuery` hook
...
-   [ ] T024 [US4] Create `src/components/messages/MessageItem.tsx` component
...
-   [ ] T025 [US4] Create `src/components/messages/MessageList.tsx` component
...
-   [ ] T026 [US4] Add scroll-to-load-more in `src/components/messages/MessageList.tsx`
...
-   [ ] T027 [US5] Create `src/lib/supabase/realtime.ts` with `subscribeToMessages(channelId, handlers)` helper
...
-   [ ] T028 [US5] Create `src/hooks/useRealtimeMessages.ts` with realtime subscription hook
...
-   [ ] T029 [US5] Implement INSERT handler in `src/hooks/useRealtimeMessages.ts`
...
-   [ ] T030 [US5] Implement UPDATE handler in `src/hooks/useRealtimeMessages.ts`
...
-   [ ] T035 [US5] Create `src/components/messages/MessageInput.tsx` component
...
-   [ ] T036 [US5] Style optimistic messages in `src/components/messages/MessageItem.tsx`
...
-   [ ] T037 [US5] Ensure dedupe handles optimistic→realtime race in `src/hooks/useRealtimeMessages.ts`
...
-   [ ] T038 [US6] Add `editMessage(messageId, body)` to `src/services/messages.ts`
    -   Validates body with `MessageBodySchema`
    -   Updates via Supabase client
    -   **Done when**: Function updates message and returns result
-   [ ] T039 [US6] Add `deleteMessage(messageId)` to `src/services/messages.ts`
    -   Deletes via Supabase client
    -   **Done when**: Function deletes message

### Edit/Delete Mutations

-   [ ] T040 [US6] Add `useEditMessage` hook to `src/hooks/useMessageMutations.ts`
...
-   [ ] T041 [US6] Add `useDeleteMessage` hook to `src/hooks/useMessageMutations.ts`
...
-   [ ] T042 [US6] Create `src/components/messages/MessageActions.tsx` component
...
-   [ ] T043 [US6] Add inline edit mode to `src/components/messages/MessageItem.tsx`
    -   Toggle between view and edit mode
    -   Save/Cancel buttons
    -   **Done when**: Can edit message inline and save
-   [ ] T044 [US6] Show "(edited)" indicator in `src/components/messages/MessageItem.tsx`
...
-   [ ] T045 Create `src/components/messages/ConnectionStatus.tsx` component
...
-   [ ] T046 Add connection status listeners in `src/hooks/useRealtimeMessages.ts`
...
-   [ ] T047 Implement reconnect reconciliation in `src/hooks/useRealtimeMessages.ts`
...
-   [ ] T048 Create `src/stores/workspace.ts` Zustand store
...
-   [ ] T049 Update `src/hooks/useRealtimeMessages.ts` to unsubscribe on channel/workspace change
...
-   [ ] T050 Clear message cache on workspace switch
...
-   [ ] T051 Create `src/lib/errors.ts` with error handling utilities
...
-   [ ] T052 Apply RLS error handling in all message mutations
...
-   [ ] T053 Create `src/app/(workspace)/[workspaceId]/[channelId]/page.tsx`
...
-   [ ] T054 Add workspace/channel validation in page component
...
-   [ ] T055 Update `docs/acceptance.md` with realtime messaging test checklist
...
-   [ ] T056 Create `tests/e2e/realtime-messages.spec.ts` with Playwright
...
-   [ ] T057 Add e2e test for optimistic rollback
...
-   [ ] T058 [P] Update `docs/rls.md` with messages RLS policy documentation
    -   Document all 4 policies with explanations
    -   **Done when**: RLS doc covers SELECT/INSERT/UPDATE/DELETE for messages
-   [ ] T059 [P] Update `docs/contracts.md` with message API contracts
